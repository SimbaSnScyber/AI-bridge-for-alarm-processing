...
var db = "axxon_psim";	// DB where to insert the data
var sqlTablename = "operator_action"; // name of the table
var sqlInstance = "DESKTOP-VSGE0M5\\SQLEXPRESS"; // SQL instance where to insert the data (FL01)
var sqlUserName = "sa";				// SQL login
    ...
    if (WorksBase64_length >= 2) { // Process the script ONLY when operator done at least one step in the workflow, ignore rest of the events. This can be achieved by checking the length of WorksBase64 array
        ...
        switch (control_type) {
            ...
            case "radio_set":                
                ...
                var child_control_id_2 = getChildControlIdValue(step_raw, "2");
                var child_control_id_3 = getChildControlIdValue(step_raw, "3");
                if(child_control_id_0 == 0){
                var edit_text = "No";
                } else {var edit_text = "Yes";}
                DebugLogString("deploy is :" +child_control_id_0)
                break;
            default:
                var edit_text = "";
                var child_control_id_0 = "";
